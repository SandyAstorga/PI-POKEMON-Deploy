const validate = (input) => {
   ...
   if (input.speed > 150 || input.speed < 1 || !/\d/g.test(input.speed)) {
      errors.speed = "The value must be between 1 and 150 defense";
   }
   if (input.weight > 150 || input.weight < 1 || !/\d/g.test(input.weight)) {
      errors.weight = "The value must be between 1 and 150 defense";
   }
   if (input.height > 150 || input.height < 1 || !/\d/g.test(input.height)) {
      errors.height = "The value must be between 1 and 150 defense";
   }   
   return errors
}
